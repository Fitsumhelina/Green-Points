...
async def profile_soil_type(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    return ADD_PROFILE
async def fetch_weather_data() -> None:
    try:
        weather_data_doc = db.collection('weather')
        if weather_data_doc.exist:
            weather_data = weather_data_doc.to_dict()
            weather_info = f"Weather Data:\n\n" \
                           f"Air Pressure at Sea Level: {weather_data.get('air_pressure_at_sea_level', 'N/A')}\n" \
                           f"Air Temperature: {weather_data.get('air_temperature', 'N/A')}\n" \
                           f"Cloud Area Fraction: {weather_data.get('cloud_area_fraction', 'N/A')}\n" \
                           f"Relative Humidity: {weather_data.get('relative_humidity', 'N/A')}\n" \
                           f"Wind From Direction: {weather_data.get('wind_from_direction', 'N/A')}\n" \
                           f"Wind Speed: {weather_data.get('wind_speed', 'N/A')}\n" \
                           f"Next 12 Hours: {weather_data.get('next_12_hours', 'N/A')}"
            return weather_info
        else:
            return "No weather data available."
    except Exception as e:
        print("Weather Data: of addis ababa \n Air Pressure at Sea Level:1009.2\n Air Temperature: 28.7\nRelative Humidity:30.3 \nWind Speed: 3\nNext 12 Hours : partly_cloudy")
        return f"An error occurred while fetching weather data: {e}"


async def general_analysis(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    weather_info = await fetch_weather_data()
    await update.message.reply_text(weather_info)

# Update Analyze Data Handler
...
# Add general_analysis handler to the application
application.add_handler(MessageHandler(filters.Regex('^General Analysis$'), general_analysis))
...
async def fertilizer_suggestions(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if crop_suggestions_docs:
        ...
        for doc in crop_suggestions_docs:
            crop_data = doc.to_dict()
            suggestions_text += f"- Farmer Name: {crop_data.get('name')}, Adviceble fertilizer: {crop_data.get('fertilizer')}, \n"
    else:
...
conv_handler = ConversationHandler( 
    ...
    states={
        ...
        REGISTER_CONTACT: [MessageHandler(filters.CONTACT, register_contact)],
        ...
        ADD_PROFILE: [
            MessageHandler(filters.Regex('^Add Profile$'), add_profile),
            MessageHandler(filters.Regex('^Main$'), start)
        ],
